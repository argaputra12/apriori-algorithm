...
def returnItemsWithMinSupport(itemSet, transactionList, minSupport, freqSet):
    # Calculating the support for items in the itemSet and returns a subset
    _itemSet = set()
...
def printResults(items, rules):
    # Print the generated itemsets sorted by support and the confidence rules sorted by confidence
...
def resultsToSring(items, rules):
    #  Print the generated itemsets sorted by support and the confidence rules sorted by confidence
    i, r = [], []
    for item, support in sorted(items, key=lambda x: x[1]):
